...
- `match-case` 基本语法：字面量匹配、通配符 `_`、捕获变量
- 序列匹配：`[first, *rest]` 解构列表，`case (x, y)` 匹配元组
- 字典匹配：`{"key": value}` 模式匹配字典结构
- 类实例匹配：`case Point(x=0, y=y)` 匹配对象属性
- 守卫条件（guard）：`case x if x > 0` 添加额外过滤
- OR 模式：`case 1 | 2 | 3` 匹配多个值
...
<CheatCard
    title="速查表"
    headers={["模式类型","语法","示例"]}
    rows={[["字面量","`case 200:`","匹配固定值"],["通配符","`case _:`","匹配任意值（兜底）"],["变量绑定","`case x:`","捕获值到变量"],["或逻辑","`case \"a\" \\","\"b\":`","多值匹配"],["序列解构","`case [x, y]:`","匹配二元组"],["剩余元素","`case [first, *rest]:`","捕获剩余"],["字典解构","`case {\"key\": value}:`","子集键匹配"],["类解构","`case Point(x, y):`","属性解构"],["守卫条件","`case x if x > 0:`","条件过滤"],["as 绑定","`case (x, y) as point:`","整体绑定"]]}
  />
## ✅ 本节总结
