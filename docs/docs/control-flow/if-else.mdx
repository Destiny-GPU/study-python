...
<CheatCard
    title="速查表"
    headers={["语法","功能","示例"]}
    rows={[["`if condition:`","条件判断","`if x > 0: print(\"正数\")`"],["`if-else`","两分支","`x if cond else y`"],["`if-elif-else`","多分支","`if/elif/else` 链式判断"],["`x if c else y`","三元运算符","`\"成年\" if age>=18 else \"未成年\"`"],["`and or not`","逻辑运算","`x > 0 and x < 100`"],["`if container:`","真值测试","`if items:` 判断非空"],["`a and b`","短路求值","`items and items[0]`"]]}
  />
## ✅ 本节总结
