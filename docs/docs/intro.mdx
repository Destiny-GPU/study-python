...
title: Python 简介
description: 了解 Python 语言的历史、特点、应用领域与版本演进，开启你的 Python 学习之旅。
---

# Python 简介

Python 是一门简洁、优雅且功能强大的通用编程语言。从 Web 开发到人工智能，从自动化脚本到科学计算，Python 几乎无处不在。本教程将带你系统学习 Python 3.12+ 的现代写法，从语法基础一路走向工程实战。

无论你是零基础的新手，还是希望从其他语言转过来的开发者，Python 都能让你在极短的时间内写出可运行的程序，并用最少的代码表达最清晰的意图。
...
Python 是一门" batteries included "（自带电池）的语言——它的标准库覆盖了网络通信、文件处理、正则表达式、数据序列化、并发编程等几乎所有常见需求，让你无需安装第三方依赖就能完成大量工作。
...
:::tip Python 之禅
在 Python 解释器中输入 `import this`，你会看到一首描述 Python 设计哲学的短诗。其中最著名的一句是：**"Simple is better than complex."（简洁胜于复杂）**。这也是本教程贯穿始终的写作理念。
:::
...
变量无需声明类型，但你可以通过**类型注解**让代码更清晰、让工具更好地帮助你。Python 3.12 引入了全新的类型参数语法（PEP 695），让泛型写法前所未有地简洁：
...
Python 同时支持面向对象、函数式和过程式编程，你可以根据场景灵活选择：
...
:::warning Python 2 已经停止维护
Python 2 已于 2020 年 1 月 1 日正式停止官方支持，所有新项目都应使用 Python 3。本教程不会涉及任何 Python 2 的遗留写法。
:::
...
本教程按照"由浅入深、边学边练"的原则组织内容，建议按顺序学习：

1. **入门准备**：搭建环境、写出第一个程序、配置开发工具
2. **基础语法**：变量、数据类型、运算符、流程控制
3. **函数与作用域**：函数定义、参数、lambda、装饰器
4. **数据结构**：列表、元组、字典、集合的进阶用法
5. **面向对象**：类、继承、数据类、协议
6. **模块与包**：import 机制、包结构、标准库巡礼
7. **异常与错误处理**：异常体系、上下文管理器
8. **文件与 IO**：文件读写、路径处理、序列化
9. **并发编程**：多线程、多进程、asyncio
10. **进阶专题**：类型系统、元编程、性能优化、测试
...
- **善用搜索**：左侧的搜索框可以快速定位你关心的知识点
- **关注提示框**：`:::tip` 是实用技巧，`:::warning` 是常见坑，`:::note` 是补充说明
- **尝试修改示例**：把示例里的数字、字符串改成你自己的，观察行为变化
- **记录学习笔记**：本站的"学习笔记"板块会持续更新一些扩展心得

:::tip 关于 Python 版本
本教程推荐使用 **Python 3.12 或更高版本**。如果你尚未安装，请先跳转到 [安装 Python 环境](./getting-started/installation.mdx) 完成环境搭建，再回到这里继续。
:::
...
:::note 接下来
环境已经就绪的同学可以直接前往 [入门准备](./getting-started/installation.mdx) 章节，正式开始动手实践。
:::
