...
import { LearningPath, TrackSelector, ProgressTracker } from '@site/src/components';
...
```mermaid
flowchart TD
    A["🐍 Python 简介"] --> B["getting-started<br/>环境搭建"]
    B --> C["basics<br/>基础语法"]
    C --> D["control-flow<br/>流程控制"]
    D --> E["data-structures<br/>数据结构"]
    E --> F["functions<br/>函数"]
    F --> G["oop<br/>面向对象"]
    G --> H["modules<br/>模块与包"]
    H --> I["error-handling<br/>异常处理"]
    I --> J["file-handling<br/>文件操作"]
    J --> K["standard-library<br/>标准库"]
    K --> L["scientific-computing<br/>科学计算"]
    L --> M["parallel-computing<br/>并行计算"]
    M --> N["advanced<br/>进阶特性"]
    N --> O["performance<br/>性能优化"]
    O --> P["testing-debugging<br/>测试与调试"]
    P --> Q["project-tutorial<br/>项目实战"]
```
