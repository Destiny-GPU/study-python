...
| 操作 | 方法 | 说明 |
|------|------|------|
| 对象→字符串 | `json.dumps(obj)` | 序列化 |
| 字符串→对象 | `json.loads(text)` | 反序列化 |
| 对象→文件 | `json.dump(obj, f)` | 写入文件对象 |
| 文件→对象 | `json.load(f)` | 从文件对象读取 |
| 中文输出 | `ensure_ascii=False` | 不转义中文 |
| 缩进美化 | `indent=2` | 2 空格缩进 |
| 键排序 | `sort_keys=True` | 输出稳定可比较 |
| 自定义类型 | `default=my_handler` | 返回可序列化值 |
| 金额精度 | `parse_float=Decimal` | 避免 float 精度丢失 |
| 忽略未知键 | `**kwargs` 解构 | 只取需要的字段 |
| 日期格式 | ISO 8601 | `datetime.isoformat()` |
| dataclass→JSON | `asdict(dc)` + `dumps` | 先转 dict 再序列化 |
| JSON→dataclass | `loads` + `**dict` | 反序列化后解构 |
