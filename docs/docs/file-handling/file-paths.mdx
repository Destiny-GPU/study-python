...
:::info[深入学习 pathlib]
**[pathlib 完整教程](../standard-library/pathlib)** — 涵盖 `PurePath` vs `Path`、`Path.walk()` 方法、Python 3.12 新特性、批量重命名实战等高级内容。
:::
...
1. 用 `pathlib` 创建一个多层目录结构 `project/src/tests`，然后用 `glob` 查找所有 `.py` 文件。
2. 给定一个文件路径，用 pathlib 属性提取文件名、扩展名、父目录，并用 `with_suffix` 将 `.txt` 改为 `.md`。

### 进阶题

3. 写一个函数，接收目录路径，递归统计该目录下所有文件的总大小（字节），并按大小降序返回前 10 个文件。
4. 用 pathlib 实现一个简单的文件备份工具：比较源目录和目标目录，找出新增和修改的文件并复制过去。
