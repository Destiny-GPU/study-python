...
CSV（Comma-Separated Values，逗号分隔值）是最简单的表格数据格式，被 Excel、数据库导出工具、数据科学工具广泛支持。Python 标准库的 `csv` 模块能正确处理各种边界情况（引号、换行、特殊字符），比手写 `split(',')` 可靠得多。本章系统介绍 CSV 的读写。
...
<CheatCard
    title="速查表"
    headers={["操作","方法","说明"]}
    rows={[["读取列表","`csv.reader(f)`","每行返回 list"],["读取字典","`csv.DictReader(f)`","每行返回 dict，首行为表头"],["写入列表","`csv.writer(f)`","`writerow()` / `writerows()`"],["写入字典","`csv.DictWriter(f, fieldnames)`","需指定 `fieldnames`"],["写表头","`writer.writeheader()`","DictWriter 专用"],["分隔符","`delimiter=','`","默认逗号，Excel 中文版常用 `;`"],["引号控制","`quoting=csv.QUOTE_MINIMAL`","默认，必要时加引号"],["全部加引号","`quoting=csv.QUOTE_ALL`","所有字段都加引号"],["数字加引号","`quoting=csv.QUOTE_NONNUMERIC`","非数字字段加引号"],["不加引号","`quoting=csv.QUOTE_NONE`","需指定 `escapechar`"],["转义字符","`escapechar='\\\\'`","QUOTE_NONE 时必需"],["打开文件","`newline=''`","避免 Windows 换行问题"],["编码","`encoding='utf-8'`","推荐，Excel 中文版用 `utf-8-sig`"],["分批读取","`itertools.islice(reader, n)`","大文件分批处理"]]}
  />
## 📚 延伸阅读
...
到这里，文件与 IO 章节就结束了。至此已掌握文件读写、路径处理、JSON 和 CSV——这四样工具足以应对 90% 的日常数据读写需求。
