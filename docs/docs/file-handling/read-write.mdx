...
:::tip Python 3.10+ 的括号写法
上面这种用括号包裹多个 `open()` 的写法从 Python 3.10 开始才被正式支持。在更早的版本中需要使用反斜杠 `\` 续行或嵌套 `with`。
:::
...
:::tip 海象运算符 :=
上面的 `while chunk := f.read(chunk_size)` 使用了 Python 3.8 引入的海象运算符，把赋值和判断合并成一行，非常优雅。
:::
