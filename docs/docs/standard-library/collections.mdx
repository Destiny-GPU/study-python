...
## 📌 本节要点
- **namedtuple**：给元组起字段名，提升可读性；新代码推荐 `@dataclass(frozen=True)`
- **deque**：双端队列，两端增删 O(1)，支持 `maxlen` 自动淘汰，适合队列/滑动窗口
- **Counter**：计数器，`most_common` 取 Top-N，支持加减与集合运算
- **defaultdict**：自动生成默认值，避免 `KeyError`，适合分组、计数、累积
- **OrderedDict**：仍用于 LRU 缓存（`move_to_end`）、顺序敏感比较
- **ChainMap**：串联多个字典，不复制数据，适合配置层级覆盖
- **UserDict/UserList/UserString**：包装类扩展更安全，避免直接继承内置类型的"快捷路径"陷阱
:::

<PyodideRunner title="collections 快速体验">
...
print(f"字符计数: {counter.most_common(3)}")
...
普通 `dict` **保证插入顺序**——这与 `OrderedDict` 行为一致。多数场景下不需要再用 `OrderedDict`：
...
直接继承 `dict` 时，`update()`、`__init__()`、`setdefault()` 等方法可能不走重写的 `__setitem__`，导致大小写转换失效：
...
标准库 `functools.lru_cache` 已实现了带装饰器的 LRU 缓存，本章实现是为展示 `OrderedDict.move_to_end` 的原理。实际开发直接用 `@lru_cache` 装饰器即可。
:::

## 🎯 动手练习

1. **词频统计**：用 `Counter` 统计一段英文文本的词频，过滤停用词后输出 Top-5
2. **滑动窗口**：用 `deque(maxlen=N)` 实现滑动窗口平均值计算
3. **配置管理**：用 `ChainMap` 实现"环境变量 > 配置文件 > 默认值"三级配置系统
4. **不区分大小写字典**：用 `UserDict` 实现键不区分大小写的字典类

<CheatCard
    title="速查表"
    headers={["容器","用途","关键特性"]}
    rows={[["`namedtuple`","具名元组","不可变、字段名访问、内存紧凑"],["`deque`","双端队列","两端 O(1)、`maxlen` 自动淘汰"],["`Counter`","计数器","`most_common`、加减运算"],["`defaultdict`","默认值字典","自动创建缺失键、避免 `KeyError`"],["`OrderedDict`","有序字典","`move_to_end`、顺序敏感比较"],["`ChainMap`","链式字典","多层级查询、不复制数据"],["`UserDict`","字典包装类","安全扩展、所有方法经过 `data`"],["`UserList`","列表包装类","安全扩展、支持自定义 `append`"],["`UserString`","字符串包装类","安全扩展、可添加自定义方法"]]}
  />
## 📚 延伸阅读

- **[collections 官方文档](https://docs.python.org/zh-cn/3/library/collections.html)** - 完整 API 参考
- **[dataclass 文档](https://docs.python.org/zh-cn/3/library/dataclasses.html)** - 现代数据类替代 namedtuple
- **[functools.lru_cache](https://docs.python.org/zh-cn/3/library/functools.html#functools.lru_cache)** - 标准库 LRU 缓存装饰器
- **[Python 字典实现原理](https://docs.python.org/zh-cn/3/c-api/dict.html)** - CPython 字典底层机制

## ✅ 本节总结

- **`namedtuple` 给元组起字段名**，提升可读性；新代码推荐 `@dataclass(frozen=True)` 替代
- **`deque` 双端队列两端增删 O(1)**，支持 `maxlen` 自动淘汰，适合队列、栈、滑动窗口
- **`Counter` 计数器**，`most_common` 取 Top-N，支持加减与集合运算（`&` 交集、`|` 并集）
- **`defaultdict` 自动生成默认值**，避免 `KeyError`，适合分组、计数、累积场景
- **`OrderedDict` 仍用于 LRU 缓存**（`move_to_end`）、顺序敏感比较；普通 `dict` 已在 3.7+ 保证插入顺序
- **`ChainMap` 串联多个字典**，不复制数据，适合配置层级覆盖（环境变量 > 配置文件 > 默认值）
- **`UserDict/UserList/UserString` 包装类扩展更安全**，所有方法都经过内部 `data`，避免直接继承内置类型的"快捷路径"陷阱

理解字典和集合的底层哈希机制，请参阅[哈希机制详解](../advanced/hash-mechanism)一章。
