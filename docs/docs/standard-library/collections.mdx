...
| 容器 | 用途 | 关键特性 |
|------|------|----------|
| `namedtuple` | 具名元组 | 不可变、字段名访问、内存紧凑 |
| `deque` | 双端队列 | 两端 O(1)、`maxlen` 自动淘汰 |
| `Counter` | 计数器 | `most_common`、加减运算 |
| `defaultdict` | 默认值字典 | 自动创建缺失键、避免 `KeyError` |
| `OrderedDict` | 有序字典 | `move_to_end`、顺序敏感比较 |
| `ChainMap` | 链式字典 | 多层级查询、不复制数据 |
| `UserDict` | 字典包装类 | 安全扩展、所有方法经过 `data` |
| `UserList` | 列表包装类 | 安全扩展、支持自定义 `append` |
| `UserString` | 字符串包装类 | 安全扩展、可添加自定义方法 |
...
理解字典和集合的底层哈希机制，请参阅[哈希机制详解](../advanced/hash-mechanism.mdx)一章。
