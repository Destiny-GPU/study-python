...
`itertools` 是 Python 中最具"函数式编程"气质的标准库。它提供一组"内存友好"的迭代器构造工具：所有函数都返回生成器，按需产出元素，可以处理超大或无限序列。掌握 `itertools` 能用极少的代码表达复杂的迭代逻辑，替代大量嵌套循环和临时列表。
...
<CheatCard
    title="速查表"
    headers={["工具","用途","关键特性"]}
    rows={[["`chain`","串联迭代器","不创建中间列表"],["`chain.from_iterable`","扁平化嵌套","O(n) 复杂度"],["`product`","笛卡尔积","`repeat=n` 自身积"],["`combinations`","无序组合","C(n, k) = n!/(k!(n-k)!)"],["`permutations`","有序排列","P(n, k) = n!/(n-k)!"],["`combinations_with_replacement`","可重复组合","C(n+k-1, k)"],["`accumulate`","累积运算","默认累加，可指定函数"],["`groupby`","相邻分组","需先排序或全局分组用 defaultdict"],["`islice`","切片迭代器","处理无限/大序列"],["`starmap`","解包参数","元组→多参数调用"],["`zip_longest`","不等长 zip","`fillvalue` 填缺失"],["`count`","无限递增","配合 islice/break"],["`cycle`","循环序列","配合 islice/break"],["`repeat`","重复对象","指定次数或无限"],["`takewhile`","取直到失败","首次失败停止"],["`dropwhile`","丢弃直到失败","之后全部保留"],["`filterfalse`","反向 filter","保留 False 的元素"],["`tee`","复制迭代器","内部缓存，注意内存"],["`batched`","分批处理","Python 3.12+ 内置"]]}
  />
## 📚 延伸阅读
