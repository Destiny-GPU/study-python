...
graph LR
    ...
    subgraph lru["@lru_cache"]
        L1["LRU 淘汰"]
        L2["可设 maxsize"]
        L3["长跑服务时用"]
    end
    subgraph prop["@cached_property"]
        P1["方法级缓存"]
        P2["计算一次后缓存"]
        P3["高成本计算时用"]
    end
...
没有 `singledispatch`，处理多种类型要么写一长串 `if isinstance(obj, ...)`，要么为每种类型定义独立函数让调用方选择。`singledispatch` 按类型分别实现，新类型可在任何地方注册，无需修改原函数——这正是"开闭原则"的体现。
:::
