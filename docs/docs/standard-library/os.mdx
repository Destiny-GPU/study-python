...
<CheatCard
    title="速查表"
    headers={["操作","方法","说明"]}
    rows={[["环境变量","`os.environ.get(key, default)`","安全读取，不存在返回默认值"],["工作目录","`os.getcwd()` / `os.chdir(path)`","多线程慎用 `chdir`"],["列举目录","`os.listdir(path)`","返回文件名列表"],["高效遍历","`os.scandir(path)`","返回 `DirEntry`，性能更优"],["递归遍历","`os.walk(path)`","返回 `(dirpath, dirnames, filenames)`"],["创建目录","`os.makedirs(path, exist_ok=True)`","递归创建，类似 `mkdir -p`"],["删除文件","`os.remove(path)`","仅删除文件"],["重命名","`os.rename(src, dst)`","可移动文件"],["执行命令","`subprocess.run([...], capture_output=True)`","推荐方式，安全"],["危险方式","`os.system(cmd)`","有命令注入风险，不推荐"],["命令行参数","`sys.argv`","`argv[0]` 是脚本名"],["退出程序","`sys.exit(code)`","0 正常，非 0 异常"],["模块路径","`sys.path`","列表，可运行时修改"],["标准流","`sys.stdin/stdout/stderr`","重定向用 `contextlib.redirect_*`"],["平台判断","`sys.platform` / `os.name`","`linux`/`darwin`/`win32`"],["Python 版本","`sys.version_info`","元组 `(major, minor, micro)`"]]}
  />
## 📚 延伸阅读
