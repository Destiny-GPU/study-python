...
异常（exception）是程序运行过程中发生的错误信号。当 Python 无法继续执行某段代码时，就会**抛出**（raise）一个异常对象；如果不处理这个异常，程序就会终止并打印错误信息。理解异常的工作机制是编写健壮代码的第一步——它使得预见错误、优雅恢复、给出有意义的反馈成为可能，而不是让程序崩溃。
...
:::info[异常不是"坏东西"]
异常不是设计缺陷，而是 Python 与开发者交流错误的方式。一个好的程序应当**主动抛出**异常来标识异常情况，并**合理处理**那些预期可能发生的异常。完全消除异常既不可能也不必要。
:::
...
```mermaid
graph TD
    Base["BaseException"] --> SE["SystemExit"]
    ...
    E --> CE["ArithmeticError"]
    CE --> ZDE["ZeroDivisionError"]
    E --> EG["ExceptionGroup"]
```
...
异常的层次结构使得既能精确捕获某个具体异常，也能捕获整组异常：
...
:::tip[智能建议]
`AttributeError` 和 `NameError` 会自动给出"你是不是想用……"的建议（如 `Did you mean: 'upper'?`），这对快速定位拼写错误非常友好。
:::
...
2. **从底部往上读调用栈**：每一段都标明"哪个文件、哪一行、哪个函数"出错
...
从下往上读：最后一行提示"找不到文件"；再往上找到出错代码位置在 `load_config` 第 2 行的 `open(path)`；再往上看，调用来源是 `main` 函数第 6 行。

## 异常即对象

异常在 Python 中就是普通的类实例对象。这意味着我们可以：
...
下面这段代码故意触发了多种常见异常，让我们尝试阅读 Traceback 并识别每种异常的类型与原因：
...
<CheatCard
    title="速查表"
    headers={["异常类型","触发场景","示例"]}
    rows={[["`ValueError`","值类型正确但内容不合法","`int(\"abc\")`"],["`TypeError`","类型不匹配","`1 + \"2\"`"],["`KeyError`","字典键不存在","`d[\"missing\"]`"],["`IndexError`","索引越界","`lst[999]`"],["`FileNotFoundError`","文件不存在","`open(\"no.txt\")`"],["`PermissionError`","权限不足","`open(\"/root/x\")`"],["`ZeroDivisionError`","除零","`5 / 0`"],["`AttributeError`","属性不存在","`\"abc\".uppercase()`"],["`NameError`","变量名未定义","`print(x)`"],["`ImportError`","模块导入失败","`import nonexistent`"],["`StopIteration`","迭代器耗尽","`next(iter([]))`"],["`RuntimeError`","一般运行时错误","自定义运行时错误"],["`NotImplementedError`","功能未实现","抽象方法占位"],["`KeyboardInterrupt`","Ctrl+C 中断","用户主动中断"],["`SystemExit`","`sys.exit()` 触发","程序主动退出"],["查看继承链","`Exception.__mro__`","完整继承顺序"],["异常属性","`exc.args`","异常参数元组"],["OSError 属性","`exc.errno`","系统错误码"],["附加说明","`exc.add_note(\"msg\")`","3.11+ 特性"]]}
  />
## ✅ 本节总结
