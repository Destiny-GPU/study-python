...
<CheatCard
    title="速查表"
    headers={["操作","语法","示例"]}
    rows={[["普通包","含 `__init__.py` 的目录","`mypackage/__init__.py`"],["子包","包内的子目录","`mypackage/sub/__init__.py`"],["公开 API","`__all__` 列表","`__all__ = [\"func1\", \"Class1\"]`"],["包版本","`__version__`","`__version__ = \"1.0.0\"`"],["当前包导入","`from . import module`","同包内模块互引"],["父包导入","`from .. import module`","导入上一级包的模块"],["绝对导入","`from pkg.sub import name`","PEP 8 推荐方式"],["命名空间包","无 `__init__.py`","多目录拼接同一命名空间"],["项目入口","`__main__.py`","`python -m mypackage`"],["可编辑安装","`pip install -e .`","开发期间源码改动立即生效"],["构建配置","`pyproject.toml`","现代 Python 项目元数据标准"],["src 布局","`src/pkgname/`","推荐用于发布库的结构"]]}
  />
## 📚 延伸阅读
