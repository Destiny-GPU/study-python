...
注释是写给人看的"代码说明"，而文档字符串（docstring）则是 Python 中一种特殊的"内置文档"机制，能为模块、函数、类、方法生成可访问的说明文档。良好的注释与文档是高质量代码的重要标志。本节我们将一起探索 Python 的注释形式、docstring 规范、Sphinx 风格，以及 `__doc__` 与 `help()` 的使用。

## 📌 本节要点
- 掌握单行注释与多行注释的正确使用方式
- 理解 docstring 的三种主流风格（reStructuredText、Google、NumPy）
- 熟练使用 `__doc__` 属性与 `help()` 函数查看文档
- 遵循"解释为什么，而非做什么"的注释原则
...
1. **编写 docstring**：为一个计算 BMI 指数的函数编写完整的 Google 风格 docstring
2. **注释重构**：将以下"做什么"注释改为"为什么"注释：`x = x + 1  # x 加 1`
3. **TODO 标记**：在一个简单的用户注册函数中添加 TODO、FIXME、NOTE 标记
4. **help() 使用**：使用 `help()` 查看 `len()`、`str.split()`、`list.append()` 的帮助信息

<PyodideRunner title="Docstring 演示">

```py
# 1. 函数的 docstring 可以通过 __doc__ 访问
def calculate_bmi(weight: float, height: float) -> float:
    """计算身体质量指数（BMI）
...
p1 = Point(0, 0)
p2 = Point(3, 4)
print(f"点(0,0) 到 点(3,4) 的距离: {p1.distance_to(p2)}")
```
...
- [PEP 257 - Docstring 规范](https://peps.python.org/pep-0257/) - Python docstring 的官方规范
- [PEP 287 - reStructuredText Docstring](https://peps.python.org/pep-0287/) - Sphinx 默认格式规范
- [Sphinx 文档](https://www.sphinx-doc.org/) - Python 最流行的文档生成工具
- [Google Python Style Guide - Comments](https://google.github.io/styleguide/pyguide.html#comments) - Google 风格的注释规范

<CheatCard
    title="速查表"
    headers={["标记","含义","使用场景"]}
    rows={[["`# 注释`","单行注释","解释复杂逻辑"],["`\"\"\"docstring\"\"\"`","文档字符串","模块/函数/类/方法说明"],["`:param x:`","参数说明","Sphinx/reStructuredText"],["`Args:`","参数说明","Google 风格"],["`:return:`","返回值","Sphinx/reStructuredText"],["`Returns:`","返回值","Google 风格"],["`:raises X:`","异常说明","Sphinx/reStructuredText"],["`TODO:`","待完成","标记未完成的功能"],["`FIXME:`","待修复","标记已知问题"],["`help(obj)`","查看帮助","交互式查询文档"]]}
  />
## ✅ 本节总结
