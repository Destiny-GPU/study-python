...
| 语法 | 含义 | 示例 |
|------|------|------|
| `name = value` | 变量赋值 | `age = 25` |
| `a, b = b, a` | 交换变量 | `x, y = y, x` |
| `first, *rest = lst` | 星号解包 | `1, [2,3,4,5]` |
| `x: int = 10` | 类型注解 | `name: str = "Alice"` |
| `int \| str` | 联合类型 | `def f(x: int \| str)` |
| `list[str]` | 泛型容器 | `names: list[str] = []` |
| `isinstance(x, T)` | 类型判断 | `isinstance(x, int)` |
| `type(x)` | 查看类型 | `type(42)  # <class 'int'>` |
