...
布尔类型（`bool`）和 `None` 是 Python 中两个非常基础但又常被误解的概念。布尔值用于表示"真"与"假"，是条件判断与逻辑运算的核心；`None` 是一个特殊的"空值"单例，表示"什么都没有"。本节我们将一起探索它们的用法、真值测试规则、短路求值，以及 `==` 与 `is` 的关键区别。

## 📌 本节要点
- 掌握 bool 类型与真值测试规则（Falsy vs Truthy）
- 理解 and/or/not 逻辑运算符与短路求值
- 熟练使用 None 与 is 运算符进行身份比较
- 区分 ==（值相等）与 is（身份相同）的本质差异
...
1. **真值测试**：列出以下值的布尔结果：`0`、`""`、`[]`、`{}`、`None`、`[0]`、`"False"`、`-1`
2. **短路求值应用**：使用 `or` 为函数参数提供默认值，使用 `and` 实现安全属性访问
3. **None 判断**：编写函数，接收任意值，判断其是否为 None，并解释为什么用 `is` 而不是 `==`
4. **逻辑运算**：不使用 `if-else`，仅用 `and/or/not` 实现一个简单的条件表达式

<PyodideRunner title="布尔值与短路求值">

```py
# 1. 真值测试：哪些值是 Falsy?
test_values = [0, 0.0, "", [], {}, set(), None, "False", "0", [0], -1]
print("=== Falsy vs Truthy ===")
for v in test_values:
    status = "Falsy" if not v else "Truthy"
    print(f"  {str(v):>10} -> {status}")

# 2. bool 是 int 的子类
print(f"\nTrue + True = {True + True}")
print(f"True * 10 = {True * 10}")
print(f"sum([True, False, True]) = {sum([True, False, True])}")
...
data = None
result = data and data.get("user")
print(f"None 安全访问: {result}")

# 5. None 判断：用 is 不用 ==
x = None
print(f"\nx is None: {x is None}")
print(f"x == None: {x == None}")
print(f"推荐用 is: {type(x) is type(None)}")
```
...
- [PEP 285 - bool 类型](https://peps.python.org/pep-0285/) - Python 布尔类型的正式规范
- [Python 真值测试文档](https://docs.python.org/zh-cn/3/library/stdtypes.html#truth-value-testing) - 官方真值规则完整说明
- [PEP 8 - 编程建议](https://peps.python.org/pep-0008/#programming-recommendations) - 关于 `is None` 的规范

<CheatCard
    title="速查表"
    headers={["语法","含义","示例"]}
    rows={[["`bool(x)`","转换为布尔值","`bool(0)` → `False`"],["`x and y`","逻辑与（短路）","`0 and 1` → `0`"],["`x or y`","逻辑或（短路）","`0 or \"default\"` → `\"default\"`"],["`not x`","逻辑非","`not True` → `False`"],["`x is None`","身份判断","`None is None` → `True`"],["`x == y`","值相等","`[1] == [1]` → `True`"],["`x is y`","身份相同","`[1] is [1]` → `False`"],["`id(x)`","对象内存地址","`id(None)`"]]}
  />
## ✅ 本节总结
