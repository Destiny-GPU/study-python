...
f-string 是最现代的字符串格式化方式，使用 `f` 前缀和 `{}` 占位符：
...
- `s[:]` 和 `s` 对于不可变类型（字符串）几乎一致，但对于可变类型（列表）`lst[:]` 创建浅拷贝。
- `split()` 不带参数按任意空白分割，是统计单词数的正确方式；`split(" ")` 会产生空字符串。
...
- 避免用 `str` 做变量名，注意字符串与代码的边界。
:::

:::info 🔗 延伸学习
想要了解更多 f-string 高级用法（如调试模式 `f"{var=}"`、格式化规范迷你语言等），请参阅 **[输入与输出](/docs/basics/input-output)** 中的 f-string 格式化章节。
:::