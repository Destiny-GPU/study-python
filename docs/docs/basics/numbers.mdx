...
数字是编程中最基础的数据类型之一。Python 提供了三种内置数字类型：**整数（int）**、**浮点数（float）** 和 **复数（complex）**，并配套了强大的算术运算与标准库模块。本节我们将一起探索数字类型的用法、运算规则以及高精度场景下的 `decimal` 与 `fractions` 模块。
...
:::tip[Python 的"大整数"特性]
在 Python 中，整数运算永远不会溢出，内存会自动扩展来容纳更大的数。这意味着我们可以直接计算 `2**1000` 这样的超大整数而无需任何特殊处理。这是 Python 与 C/Java 等语言在整数处理上的本质差异。
:::
...
<CheatCard
    title="速查表"
    headers={["语法/函数","功能","示例"]}
    rows={[["`int`","整数（无大小限制）","`10**100`"],["`float`","浮点数（IEEE 754）","`3.14`, `1e-5`"],["`complex`","复数","`3+4j`"],["`//`","整除（向下取整）","`7 // 2` → `3`"],["`%`","取模","`7 % 3` → `1`"],["`**`","幂运算","`2 ** 10` → `1024`"],["`divmod(a, b)`","商和余数","`divmod(17, 5)` → `(3, 2)`"],["`round(x, n)`","四舍五入（银行家舍入）","`round(2.5)` → `2`"],["`bin(x)`","转二进制字符串","`bin(10)` → `'0b1010'`"],["`Decimal(\"0.1\")`","精确十进制","`Decimal(\"0.1\") + Decimal(\"0.2\")` → `0.3`"],["`Fraction(1, 3)`","精确分数","`Fraction(1, 2) + Fraction(1, 3)` → `5/6`"]]}
  />
## ✅ 本节总结
