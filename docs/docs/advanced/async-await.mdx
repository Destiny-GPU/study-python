...
:::warning 协程不调用就不会执行
协程函数调用后返回协程对象，但**不会自动执行**。必须有 `await` 或事件循环驱动它。如果你写了 `async def` 却没 `await`，会看到 "coroutine was never awaited" 警告。
:::
...
- `task = create_task(coro()); await task`：任务立即开始跑，你可以稍后再等——**并发**
