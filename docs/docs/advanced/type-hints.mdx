...
1. **类型注解实践**：为之前编写的传感器处理函数添加完整的类型注解，包括参数和返回值
...
<CheatCard
    title="速查表"
    headers={["语法","含义","Python 版本"]}
    rows={[["`x: float`","变量类型注解","3.5+"],["`def f() -> float:`","函数返回类型","3.5+"],["`list[float]`","泛型容器","3.9+"],["`int \\","str`","联合类型","3.10+"],["`X \\","None`","可选类型","3.10+"],["`type SensorID = str`","类型别名","3.12+"],["`def f[T](x: T) -> T:`","泛型函数","3.12+"],["`TypeGuard[T]`","类型守卫","3.10+"],["`isinstance(x, T)`","运行时类型检查","所有版本"]]}
  />
## ✅ 本节总结
