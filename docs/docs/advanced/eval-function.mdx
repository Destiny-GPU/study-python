...
<CheatCard
    title="速查表"
    headers={["操作","方法","安全性","适用场景"]}
    rows={[["危险解析","`eval(s)`","❌ 危险","禁止用于用户输入"],["安全解析","`ast.literal_eval(s)`","✅ 安全","字面量：列表、字典、元组"],["JSON 解析","`json.loads(s)`","✅ 安全","JSON 格式数据"],["字符串分割","`s.split(',')`","✅ 安全","简单分隔符数据"],["类型转换","`int(s)` / `float(s)`","✅ 安全","单个数值转换"]]}
  />
## 📚 延伸阅读
