...
[结构化模式匹配](../control-flow/match-case)介绍了 `match-case` 的基础语法。本节深入模式匹配的进阶用法——`*rest` 与 `**rest` 收集模式、嵌套解构、OR 模式组合、`as` 捕获、Python 3.12 增强，以及实战中的 AST 节点处理。掌握这些技巧后，你能用声明式风格优雅地处理复杂结构化数据，替代大量手写 `if-else` 与索引访问。
...
<CheatCard
    title="速查表"
    headers={["模式","语法","说明"]}
    rows={[["字面量","`case 0:`","匹配具体值"],["捕获","`case x:`","绑定任意值到变量"],["通配符","`case _:`","匹配任意值，不绑定"],["值模式","`case Color.RED:`","匹配常量/枚举"],["序列","`case [a, b, *rest]:`","解构列表/元组"],["映射","`case {\"key\": val, **rest}:`","解构字典，子集匹配"],["类位置","`case Point(x, y):`","需 `__match_args__`"],["类关键字","`case Point(x=x, y=y):`","更通用，推荐"],["OR","`case \"a\" \\","\"b\":`","合并多个值"],["as 捕获","`case [x, y] as lst:`","绑定整体"],["守卫","`case x if x > 0:`","条件过滤"],["嵌套","`case {\"users\": [{\"name\": n}]}:`","深度解构"]]}
  />
## 📚 延伸阅读
