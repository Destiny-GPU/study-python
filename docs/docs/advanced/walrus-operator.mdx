...
海象运算符 `:=`（walrus operator，PEP 572）是 Python 3.8 引入的**赋值表达式**语法。它让"赋值"可以作为表达式出现在原本不允许的位置——`if` 条件、`while` 循环、列表推导式等。这个看似微小的语法改进，能消除大量"先赋值、再判断"的重复代码，是现代 Python 代码风格的重要组成。
