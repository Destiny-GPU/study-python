...
| 场景 | 传统写法 | 海象运算符写法 |
|------|---------|---------------|
| if 条件 | `n = func(); if n > 0:` | `if (n := func()) > 0:` |
| while 循环 | `line = read(); while line:` | `while (line := read()):` |
| 推导式 | `[f(n) for n in nums if f(n) > 0]` | `[y for n in nums if (y := f(n)) > 0]` |
| 正则匹配 | `m = re.match(...); if m:` | `if (m := re.match(...)):` |
| 字典 get | `v = d.get(k); if v:` | `if (v := d.get(k)):` |
| 异常信息 | `except Exception as e:` | 不适用（已有 as 语法） |
