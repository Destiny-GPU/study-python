...
- 列表推导式 `[expr for x in iter if cond]` 比 `for + append` 更快更简洁
- 字典推导式 `{k: v for ...}` 和集合推导式 `{x for ...}` 同理
- 生成器表达式 `(x for x in ...)` 惰性求值，适合大数据流
- `if` 在 `for` 后是过滤，`if-else` 在表达式里是选择，注意区别
- 嵌套推导式超过两层建议改用显式循环，保持可读性

<PyodideRunner title="推导式快速体验">
...
# 字典推导式
word_lengths = {word: len(word) for word in ["python", "java", "c++", "javascript"]}
print(f"单词长度: {word_lengths}")

# 集合推导式
first_letters = {word[0] for word in ["apple", "avocado", "banana", "cherry"]}
print(f"首字母集合: {first_letters}")

# 生成器表达式（节省内存）
total = sum(x ** 2 for x in range(1000000))
print(f"1-100万的平方和: {total}")
```
...
<Tabs>
  <TabItem value="traditional" label="传统写法">
...
:::info[海象运算符 :=]
`:=`（海象运算符）可以在表达式内赋值。它有合理用途（如缓存计算结果避免重复调用），但**不要**用来在推导式里搞副作用编程，那会降低可读性。
:::
...
1. **列表推导式**：使用推导式生成 1-100 中所有能被 3 或 5 整除的数（FizzBuzz 前置）
2. **字典推导式**：给定单词列表 `["apple", "banana", "cherry"]`，创建 `{单词: 长度}` 的字典
3. **生成器表达式**：使用生成器表达式计算 1-1000 的平方和，对比与列表推导式的内存占用
4. **矩阵转置**：使用 `zip(*matrix)` 和嵌套推导式两种方式实现 3x4 矩阵转置

## 📚 延伸阅读

- [PEP 202 - 列表推导式](https://peps.python.org/pep-0202/) - 列表推导式规范
- [PEP 274 - 字典推导式](https://peps.python.org/pep-0274/) - 字典推导式规范
- [PEP 289 - 生成器表达式](https://peps.python.org/pep-0289/) - 生成器表达式规范
- [Python 函数式编程指南](https://docs.python.org/zh-cn/3/howto/functional.html) - map/filter/推导式对比

<CheatCard
    title="速查表"
    headers={["类型","语法","示例"]}
    rows={[["列表推导式","`[expr for x in iter]`","`[x**2 for x in range(5)]`"],["字典推导式","`{k: v for ...}`","`{x: x**2 for x in range(5)}`"],["集合推导式","`{expr for ...}`","`{x**2 for x in range(5)}`"],["生成器表达式","`(expr for ...)`","`(x**2 for x in range(5))`"],["条件过滤","`[x for x if cond]`","`[x for x in range(10) if x%2==0]`"],["条件选择","`[a if c else b for ...]`","`[\"偶\" if x%2==0 else \"奇\" for x in range(5)]`"],["嵌套推导式","`[expr for x for y]`","`[x for row in matrix for x in row]`"],["zip 配对","`[(a,b) for a,b in zip()]`","`[(n,s) for n,s in zip(names,scores)]`"],["聚合函数","`sum/gen`","`sum(x**2 for x in range(10))`"]]}
  />
## ✅ 本节总结
