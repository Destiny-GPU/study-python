...
| 字典推导式 | `{k: v for ...}` | `{x: x**2 for x in range(5)}` |
| 集合推导式 | `{expr for ...}` | `{x**2 for x in range(5)}` |
| 生成器表达式 | `(expr for ...)` | `(x**2 for x in range(5))` |
| 条件过滤 | `[x for x if cond]` | `[x for x in range(10) if x%2==0]` |
| 条件选择 | `[a if c else b for ...]` | `["偶" if x%2==0 else "奇" for x in range(5)]` |
| 嵌套推导式 | `[expr for x for y]` | `[x for row in matrix for x in row]` |
| zip 配对 | `[(a,b) for a,b in zip()]` | `[(n,s) for n,s in zip(names,scores)]` |
| 聚合函数 | `sum/gen` | `sum(x**2 for x in range(10))` |
