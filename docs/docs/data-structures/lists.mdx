...
列表推导式是 Python 中创建列表的优雅写法，本节先做铺垫，详细内容见[推导式](./comprehensions.mdx)一章。
...
4. **性能对比**：使用 `timeit` 对比 `list.insert(0, x)` 和 `deque.appendleft(x)` 的性能差异
...
| 操作 | 方法/语法 | 时间复杂度 |
|------|-----------|------------|
| 创建列表 | `[1, 2, 3]` 或 `list(iterable)` | O(n) |
| 索引访问 | `lst[i]` | O(1) |
| 切片 | `lst[start:stop:step]` | O(k) |
| 末尾追加 | `lst.append(x)` | O(1) |
| 末尾弹出 | `lst.pop()` | O(1) |
| 任意插入 | `lst.insert(i, x)` | O(n) |
| 按值删除 | `lst.remove(x)` | O(n) |
| 成员查找 | `x in lst` | O(n) |
| 排序 | `lst.sort()` 或 `sorted(lst)` | O(n log n) |
| 反转 | `lst.reverse()` 或 `lst[::-1]` | O(n) |
| 浅拷贝 | `lst.copy()` 或 `lst[:]` | O(n) |
| 深拷贝 | `copy.deepcopy(lst)` | O(n) |
