...
| 操作 | 语法 | 示例 |
|------|------|------|
| 创建集合 | `{a, b}` 或 `set()` | `s = {1, 2, 3}` |
| 空集合 | `set()` | `empty = set()`（不是 `{}`） |
| 添加元素 | `s.add(x)` | `s.add(4)` |
| 删除元素 | `s.discard(x)` | 不存在不报错 |
| 删除元素 | `s.remove(x)` | 不存在抛 `KeyError` |
| 弹出元素 | `s.pop()` | 弹出任意一个 |
| 并集 | `a | b` 或 `a.union(b)` | `{1,2} | {2,3}` → `{1,2,3}` |
| 交集 | `a & b` 或 `a.intersection(b)` | `{1,2} & {2,3}` → `{2}` |
| 差集 | `a - b` 或 `a.difference(b)` | `{1,2} - {2,3}` → `{1}` |
| 对称差 | `a ^ b` 或 `a.symmetric_difference(b)` | `{1,2} ^ {2,3}` → `{1,3}` |
| 子集判断 | `a <= b` 或 `a.issubset(b)` | `{1} <= {1,2}` → `True` |
| 不可变集合 | `frozenset(iterable)` | `fs = frozenset([1,2,3])` |
