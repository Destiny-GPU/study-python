...
集合（`set`）是 Python 中**无序、不重复**的可变容器。它基于哈希表实现，最大特点是**成员测试 O(1)**与**自动去重**，并提供并、交、差、对称差等数学集合运算。当我们可以"判断某元素是否出现过"或"求两个群体的交集"时，集合几乎总是最佳选择。

## 📌 本节要点

- 集合去重：`set(lst)` 自动去除重复元素，保持唯一性
- 成员测试 O(1)：`x in s` 比列表快得多，适合频繁查找
- 集合运算：`&` 交集、`|` 并集、`-` 差集、`^` 对称差集
- 集合推导式 `{x for x in ...}` 和 `frozenset` 不可变集合
- `setdefault()` 和集合运算在数据清洗、权限管理中的应用

<PyodideRunner title="集合操作快速体验">

```py
# 集合运算
frontend_devs = {"Alice", "Bob", "Carol", "David"}
backend_devs = {"Bob", "David", "Eve", "Frank"}

# 交集：全栈开发者
fullstack = frontend_devs & backend_devs
print(f"全栈开发者: {fullstack}")

# 并集：所有开发者
all_devs = frontend_devs | backend_devs
print(f"所有开发者: {all_devs}")

# 差集：只做前端
frontend_only = frontend_devs - backend_devs
print(f"只做前端: {frontend_only}")

# 对称差集：不是全栈
not_fullstack = frontend_devs ^ backend_devs
print(f"不是全栈: {not_fullstack}")

# 去重
numbers = [1, 2, 2, 3, 3, 3, 4, 4, 4, 4]
unique = list(dict.fromkeys(numbers))  # 保序去重
print(f"\n原始列表: {numbers}")
print(f"去重后: {unique}")
```
...
:::tip[何时用 frozenset]
当我们可以把"一组值"用作字典键、集合元素，或想表达"集合内容不可变"的契约时，用 `frozenset`。它和 `tuple` 之于 `list` 的关系类似。
:::
...
1. **去重对比**：分别使用 `set()`、`dict.fromkeys()` 和循环实现列表去重，对比性能和保序性
2. **集合运算**：给定两个用户列表，找出共同用户、独有用户，并统计各自数量
3. **frozenset 实践**：使用 `frozenset` 作为字典键存储一组标签，演示顺序无关性
4. **性能测试**：使用 `timeit` 对比 10 万元素列表和集合的成员查找性能差异

## 📚 延伸阅读

- [Python 集合文档](https://docs.python.org/zh-cn/3/library/stdtypes.html#set) - 官方集合类型详解
- [frozenset 文档](https://docs.python.org/zh-cn/3/library/stdtypes.html#frozenset) - 不可变集合详解
- [集合运算数学基础](https://docs.python.org/zh-cn/3/library/stdtypes.html#set-types-set-frozenset) - 并交差对称差运算
- [性能优化指南](https://wiki.python.org/moin/TimeComplexity) - Python 数据结构时间复杂度

<CheatCard
    title="速查表"
    headers={["操作","语法","示例"]}
    rows={[["创建集合","`{a, b}` 或 `set()`","`s = {1, 2, 3}`"],["空集合","`set()`","`empty = set()`（不是 `{}`）"],["添加元素","`s.add(x)`","`s.add(4)`"],["删除元素","`s.discard(x)`","不存在不报错"],["删除元素","`s.remove(x)`","不存在抛 `KeyError`"],["弹出元素","`s.pop()`","弹出任意一个"],["并集","`a","b` 或 `a.union(b)`","`{1,2}","{2,3}` → `{1,2,3}`"],["交集","`a & b` 或 `a.intersection(b)`","`{1,2} & {2,3}` → `{2}`"],["差集","`a - b` 或 `a.difference(b)`","`{1,2} - {2,3}` → `{1}`"],["对称差","`a ^ b` 或 `a.symmetric_difference(b)`","`{1,2} ^ {2,3}` → `{1,3}`"],["子集判断","`a <= b` 或 `a.issubset(b)`","`{1} <= {1,2}` → `True`"],["不可变集合","`frozenset(iterable)`","`fs = frozenset([1,2,3])`"]]}
  />
## ✅ 本节总结
