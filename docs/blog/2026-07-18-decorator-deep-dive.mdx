...
装饰器是 Python 里「能让你代码立刻看起来很专业」的特性，也是初学者最容易卡住的坎。

我第一次学装饰器时，看了一堆教程，每个都说「装饰器就是高阶函数」，然后就跳到了代码。我当时：？？？到底什么是高阶函数？为什么加了个 `@` 就变了？后来我想通了一件事——**装饰器不是语法问题，是思维问题**。一旦你接受「函数就是普通的值」，所有装饰器语法都顺理成章。这篇笔记就是把这个「想通」的过程写下来。
...
:::note
理解装饰器的关键，不是去记 `@decorator` 的语法，而是先接受「函数就是普通的值」。后面所有装饰器都是这个事实的自然推论。如果你现在还觉得函数很「特殊」，试着把它想象成一个整数——你可以把整数赋给变量、传给函数、放进列表，函数也一样。
:::
...
:::warning
很多教程只讲到这里就停了，但你立刻会发现一个坑：`hello` 的 `__name__` 变成了 `wrapper`，`help(hello)` 也丢了原函数的文档。我第一次写装饰器就踩了这个坑——debug 的时候发现函数名全变成 `wrapper`，差点崩溃。这就是 `functools.wraps` 存在的理由——下一节讲。
:::
...
**把它当成写装饰器的肌肉记忆**——就像写 `__init__` 时总会写 `self` 一样，写 wrapper 时总会加 `@wraps`。
...
记忆要点：**普通装饰器是两层函数，带参数装饰器是三层函数**。画一下调用栈就清楚了：

```
repeat(3)          → 返回 decorator
decorator(ping)    → 返回 wrapper
wrapper()          → 循环调用 ping 3 次
```
...
我个人更喜欢用函数装饰器，因为更轻量。但在需要维护调用计数、缓存状态这类场景下，类装饰器的可读性确实更好——状态直接挂在 `self` 上，不用费劲地用 `nonlocal`。
...
:::tip
3.9+ 推荐用 `functools.cache`，它等价于 `lru_cache(maxsize=None)`，无大小限制且略快。3.12 还新增了 `@override` 用于标注重写。这些内置装饰器用好了，能省掉你自己写大量样板代码。
:::
